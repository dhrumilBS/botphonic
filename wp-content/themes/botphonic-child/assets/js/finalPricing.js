...
function renderShortPlans() {
	...
	isRecommendedBadge = (isRecommended == true) ? `<div class="highlted-badge"> <span class="rounded-pill px-3 py-1 "> Recommnded </span> </div>` : '';		

	planHTML = `
<div class="short-plan-price">
<div class="short-plan-final"> <span class="symbol">${symbol[shortCurrency]}</span> <span class="yearlyPrice h2">${yearlyPrice.toFixed(0)}</span> <small class="perMonth">/mo</small></div>
${shortBillingPeriod === "Yearly" && monthlyPrice > 0 && monthlyPrice !== yearlyPrice
		? `<span class="d-inline short-plan-strike old-price"><span class="symbol">${symbol[shortCurrency]}</span> <span class="monthlyPrice">${monthlyPrice}/mo</span></span>`
	: ""}
</div>`;

	// discount = discount > 0 ? `<div class="short-plan-badge">Save <span class="symbol">${symbol[shortCurrency]}</span> ${(expectedYearly - discount).toFixed(2)}</div>` : ""
...
document.addEventListener("DOMContentLoaded", () => {
	...
	function updateUI() {
		toggle.style.left = isYearly ? "20px" : "4px";
